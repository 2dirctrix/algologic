<template>
  ...
  >
    ...
    <img :src="iconPath" :alt="iconType" class="spell-icon" @error="onImageError" />
  </div>
</template>
...
const iconPath = computed(() => {
  try {
    ...
    return new URL(`/src/game/assets/spell-icon/${props.iconType}.png`, import.meta.url).href;
  } catch {
    console.warn(`Spell icon not found: ${props.iconType}.png`);
    return '';
  }
});
...
const handleDragStart = e => {
  ...
  emit('dragStart', { type: 'spell', spellId: props.spellId, iconType: props.iconType });
};
...
const onImageError = () => {
  console.warn(`Failed to load spell icon: ${props.iconType}.png`);
};
</script>
...
.spell-card {
  ...
  &.dragging {
    ...
    cursor: grabbing;
    border-color: $electric-violet;
    box-shadow: 0 0 1vh rgba($electric-violet, 0.5);
  }
  ...
  .remaining-count {
    ...
    width: 2.5vh;
    height: 2.5vh;
    background: rgba($electric-violet, 0.9);
    color: $text-primary;
    border-radius: 50%;
    display: flex;
    align-items: center;
