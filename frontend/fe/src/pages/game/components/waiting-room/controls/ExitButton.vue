<template>
  <button class="exit-button" @click="handleClick" @mouseenter="handleMouseEnter">나가기</button>
</template>
...
.exit-button {
  background: linear-gradient(135deg, 
    rgba($cyberpunk-pink, 0.2) 0%, 
    rgba($cyberpunk-pink, 0.1) 50%, 
    rgba($card-bg, 0.8) 100%
  );
  border: calc(0.3vh) solid rgba($cyberpunk-pink, 0.8);
  ...
  text-shadow: 0 0 0.8vh rgba($cyberpunk-pink, 0.6);
  ...
  &:hover {
    background: linear-gradient(135deg, 
      ...
      rgba($cyberpunk-pink, 0.15) 50%, 
      rgba($card-bg, 0.9) 100%
    );
    border-color: $cyberpunk-pink;
    color: lighten($cyberpunk-pink, 30%);
