<template>
  <div class="spell-card" :class="{ selected: isSelected, disabled }" @click="handleClick" @mouseenter="handleMouseEnter">
    <div class="spell-card__icon">
      <img :src="spellIconSrc" :alt="spell.spellName" @error="handleImageError" />
    </div>
    <div class="spell-card__info">
      <div class="spell-card__name">{{ spell.spellName }}</div>
      <div class="spell-card__cost">{{ spell.cost }}G</div>
    </div>
  </div>
</template>
...
// Computed
const spellIconSrc = computed(() => {
  if (imageError.value) {
    // 기본 스펠 아이콘으로 대체
    return new URL(`/src/game/assets/spell-icon/default-spell.png`, import.meta.url).href;
  }
  return new URL(`/src/game/assets/spell-icon/spell-${props.spell.spellId}.png`, import.meta.url).href;
});
